...
class PushDocumentAddScreen extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            doctitle: '',
            metadata: '',
            dockey: this.props.dockey ? this.props.dockey : '',
            myObj: [],
            holderKey: '',
            holderValue: '',
            index: 0,
    ...
    addTextInput = (index) => {
        ...
        textInput.push(
            <View key={index}>
                ...
                /> */}
                <TextInput style={styles.inputText} onChangeText={(text) => { this.setState({ holderKey: text })                                                                               
                                                                            }} 
                ...
                />                 */}
                <TextInput style={styles.inputText} onChangeText={(text) => { this.setState({ holderValue: text })
                                                                            }} 
                                                    onEndEditing={()=>{ this.pushToArray(index) }}/>
            </View>);
        this.setState({ textInput })
    ...
    render() {
        ...
        let serviceRepoID = this.state.dsRepoID.map((item, key) => {
            return <Picker.Item key={key} value={item.repoID} label={item.repoID} />
        });
