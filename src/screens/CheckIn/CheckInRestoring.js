...
import { unsealedPull,dropInit } from '../../actions/pull'
...
class CheckInRestoringScreen extends Component {
    constructor(props) {
        super(props)
        this.state = {
           secretKey: '',
    ...
    RestoreInit() {
        var url = connection.SERVER + 'checkin/restore/init'
        //var url = 'http://192.168.33.48:5005/push/app'
        console.log("URL=" + url)
        var formData = new FormData();
        formData.append('token', this.props.token);
        formData.append('checkinkey', this.props.pullKey);
            ...
            fetch(url, config)
                ...
                .then((responseJson) => {
                    if (responseJson.status === true) {
                        ...
                        //this.props.unsealedPull(this.props.token, this.state.secretKey,'restore');
                        this.props.dropInit(this.props.token, this.props.pullKey ,this.state.secretKey,'restore')
    ...
    onValueChangeApprove(value) {
     ...
     //CheckIn
     Restore(){ 
        var url = connection.SERVER + 'checkin/restore'
        console.log("URL=" + url)
        var formData = new FormData();
        ...
        return (
            <ScrollView style={styles.container}>
                <ContainerView>
                    ...
                    <ButtonContainer>
                        <Button text="Restore" onPress={() => { this.RestoreInit() }} />
                        <Button text="Cancel" onPress={() => { this.props.navigation.navigate('CheckInRestoringList') }} />
                    </ButtonContainer>
                </ContainerView>
            </ScrollView>
        );
...
export default connect(mapStateToProps, mapDispatchToProps)(CheckInRestoringScreen);