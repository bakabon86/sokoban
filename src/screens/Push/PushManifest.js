...
class PushManifestScreen extends Component {
    constructor(props) {
        super(props)
        this.state = {
            since: dateNow,
        ...
        fetch(url, config)
            ...
            .then((responseJson) => {
                if (responseJson.status === true) {
                    ...
                    if( this.props.returning === true ){
                        this.getRepoIDReturnable();
                    }
                    else
                    {
                        this.getRepoIDPushable();
                    }
    ...
    getRepoIDReturnable(){
        var url = connection.SERVER + 'repo/list/returnable?token=' + this.props.token +
                    '&trxkey=' + this.props.pushKey
        console.log("URL=" + url);
        const config = {
            method: 'POST',
    ...
    pushManifestDelete() {
            ...
            console.log('RepoID=' +this.props.repoID)
            const config = {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
        ...
        return (
            <ScrollView style={styles.container}>
                <ContainerView>
                    ...
                    <Text style={styles.labelText}>Add New Repo :</Text>
                    <Picker
...
export default connect(mapStateToProps, mapDispatchToProps)(PushManifestScreen);