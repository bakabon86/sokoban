...
class PushVerifiedListScreen extends Component {
    constructor(props) {
        super(props)
        this.state={
            // dsInstance: [],
    ...
    getPushVerifiedList() {
       if(this.state.instance !== ''){
        var url = connection.SERVER + 'push/VerifiedList?token=' + this.props.token 
        ...
        return fetch(url,config)
            ...
            .then((responseJson) => {
                if (responseJson.status === true) {
                    this.setState({
                        dsVerifiedList: responseJson.data
                    })
                }
                else {
                    this.setState({
                ...
                <ContainerView>
                    ...
                    <ScrollView horizontal={true}>
                        <View style={{ justifyContent: 'flex-start' }}>
                            ...
                            <FlatList
                                data={this.state.dsVerifiedList}
                                renderItem={({ item }) => 
                                    <TouchableOpacity onPress={() => {
                                       
                                         this.props.Packed(this.props.token, item.repoID);
                                         this.props.navigation.navigate("PushPacked", { key: item.key });
                                    }}>
                                        <View style={styles.tableContainer}>
                                            {/* <View style={styles.tableContent}>
                                                <Text>{item.key} </Text>
...
export default connect(mapStateToProps,mapDispatchToProps)(PushVerifiedListScreen);