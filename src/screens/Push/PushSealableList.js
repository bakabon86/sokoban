...
import { sealPush } from '../../actions/push';
...
class PushSealableListScreen extends Component {
    constructor(props) {
        super(props)
        this.state = {
            dsPushSealableList: [],
    ...
    getPushSealableList() {
        ...
        var url = connection.SERVER + 'push/list/sealable?token=' + this.props.token
        console.log("URL=" + url)
        const config = {
            method: 'POST',
            headers: {
        ...
        return fetch(url,config)
            ...
            .then((responseJson) => {
                if (responseJson.status === true) {
                    this.setState({
                        dsPushSealableList: responseJson.data
                    })
                }
                else {
            ...
            <ScrollView horizontal={true}>
                <ContainerView>
                    ...
                    <View style={{ justifyContent: 'flex-start' }}>
                        ...
                        <FlatList
                            data={this.state.dsPushSealableList}
                            renderItem={({ item }) => //<View style={{ flex: 1, flexDirection: 'row',justifyContent:'flex-start' }}>
                                <TouchableOpacity onPress={() => {
                                    //alert(item.nik + ',' + item.name);
                                    this.props.sealPush(this.props.token, item.key, item.number, item.trxtype, item.instance, 
                                    ...
                                    this.props.navigation.navigate("PushSeal", { key: item.key });
                                }}>
                                    <View style={styles.tableContainer}>
                                        {/* <View style={styles.tableContent}>
                                            <Text>{item.key} </Text>
...
const mapDispatchToProps = (dispatch) => {
    return {
        sealPush: (token, key, number, trxtype, instance, repocount, requested,requestedby,requestedbyNIK,approved,approvedby,approvedbyNIK,accepted,acceptedby,acceptedbyNIK) => {
            dispatch(sealPush(token, key, number, trxtype, instance, repocount, requested,requestedby,requestedbyNIK,approved,approvedby,approvedbyNIK,accepted,acceptedby,acceptedbyNIK))
        }
    };
}
export default connect(mapStateToProps, mapDispatchToProps)(PushSealableListScreen);