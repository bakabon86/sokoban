...
class PushApprovalScreen extends Component {
    constructor(props) {
        super(props)
        this.state = {
            pushKey: '',
    ...
    Approve() {
        var url = connection.SERVER + 'trx/approve'
        //var url = 'http://192.168.23.105:5005/trx/approve'
            ...
            fetch(url, config)
                ...
                .then((responseJson) => {
                    if (responseJson.status === true) {
                        ...
                        alert("Approval has been Saved");
                       
                        this.props.navigation.navigate("PushApprovalList");
                    }
                    else {
        ...
        return (
            <ScrollView style={styles.container}>
                <ContainerView>
                    ...
                    <Text style={styles.labelText}>Trx Ops :</Text>
                    <TextInput style={styles.labelDefaultText} editable={false}
                    ...
                    <Text style={styles.labelText}>Exec Date :</Text>
                    <TextInput style={styles.labelDefaultText} editable={false}
                    ...
                    <ButtonContainer>
                        ...
                        <Button text="Cancel" onPress={() => { this.props.navigation.navigate('PushApprovalList') }} />
                    </ButtonContainer>
                </ContainerView>
            </ScrollView>
        );
...
const mapStateToProps = (state) => {
    return {
        ...
        pushname: state.reducerPush.name,
        pushnik: state.reducerPush.nik,
        ops: state.reducerPush.ops,
        reason: state.reducerPush.reason
    };
}
export default connect(mapStateToProps)(PushApprovalScreen);