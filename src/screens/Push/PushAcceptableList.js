...
class PushAcceptableListScreen extends Component {
    constructor(props) {
        super(props)
        this.state = {
            dsPushAcceptableList: [],
            trxtype: ''
        }
    }
    ...
    getPushAcceptableList(trxtype) {
        //var url = connection.SERVER + 'register/apprlist?token=' + this.props.token
        var url = connection.SERVER + 'trx/list/acceptable?token=' + this.props.token
        console.log("URL=" + url)
        const config = {
            method: 'POST',
            headers: {
        ...
        return fetch(url,config)
            ...
            .then((responseJson) => {
                if (responseJson.status === true) {
                    if (trxtype === '') {
                        this.setState({ dsPushAcceptableList: responseJson.data })
                    }
                    else {
                        const tipe = responseJson.data.filter(x => x.trxtype === trxtype)
                        this.setState({
                            dsPushAcceptableList: tipe
                        })
                    } 
                }
                else {
    ...
    bindDataTable() {
        this.setState({ dsPushAcceptableList: [] });
        this.getPushAcceptableList(this.state.trxtype);
    }
            ...
            <ScrollView style={styles.container}>
                <ContainerView>
                    ...
                    <View style={{ justifyContent: 'flex-start' }}>
                        ...
                        <FlatList
                            data={this.state.dsPushAcceptableList}
                            renderItem={({ item }) => //<View style={{ flex: 1, flexDirection: 'row',justifyContent:'flex-start' }}>
                                <TouchableOpacity onPress={() => {
                                    // alert(item.nik + ',' + item.name);
                                     ...
                                     this.props.navigation.navigate("PushAcceptable", { key: item.key });
                                }}>
                                    <View style={styles.tableContainer}>
                                        {/* <View style={styles.tableContent}>
                                            <Text>{item.key} </Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(PushAcceptableListScreen);