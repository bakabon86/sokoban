...
import { tryPacking, storeInit, acceptedPush, sealedPush } from '../actions/push';
import { dropInit } from '../actions/pull';
...
class VerificationInitiatorScreen extends Component {
            ...
            .then((responseJson) => {
                if (responseJson.status === true) {
                    ...
                    if (this.state.stepNow === 'seal') {
                        ...
                        this.Store();
                        // this.props.storeInit(this.props.token,this.state.trxKey,this.state.secretKey,'','store')
                        // this.props.navigation.navigate("BeginVerification");
                        //this.props.navigation.navigate('PushStore')
                    }
                    else if (this.state.stepNow === 'drop'){
                        this.Drop();
                    }
                    else if(this.state.stepNow === 'unseal'){
                        this.UnSeal();
                    }
                    else if (this.state.stepNow === 'restore'){
                        this.Restore();
                    }
                    else if(this.state.stepNow === 'pick'){
                        this.Picking();
                    }
                    else if(this.state.stepNow === 'burn'){
                        this.Burn();
                    }
                }
                else {
        ...
        fetch(url, config)
            ...
            .then((responseJson) => {
                if (responseJson.status === true) {
                    ...
                    // alert("Push has been Sealed");
                    this.setState({ secretKey: responseJson.key });
                    this.props.dropInit(this.props.token,this.state.trxKey,this.state.secretkey,this.state.stepNow)
                    // this.props.navigation.navigate("TwoWayVerificationInitiator");
                    this.props.navigation.navigate("BeginVerification");
                }
                else {
    ...
    Burn(){
        var url = connection.SERVER + 'dispose/burn'
        console.log("URL=" + url)
        var formData = new FormData();
        formData.append('secretkey', this.state.secretKey);
